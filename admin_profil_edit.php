...
if(isset($_POST['simpan'])){
 ...
 $query = mysql_query("update profil set judul_artikel='".$_POST['judul']."', isi_artikel='".$isi."', tgl_terbit_artikel='".date('Y-m-d H:i:s')."' where id_artikel='".$_GET['id']."'");
 if($query){
?>
<script language="javascript">
	alert("data tersimpan");
 urlini = document.location.href.split('/');
	document.location = "http://"+urlini[2]+"/geocoder/admin_profil_lihat.php";
</script>
<?php
 } else {
  echo mysql_error();
...
<input type="hidden" name="tgl" value="<?= date('Y-m-d H:i:s') ?>" />
 <div class="form-group">
  ...
  <div class="col-sm-4">
   <input type="text" name="judul" id="judul" class="form-control" value="<?= $row['judul_artikel'] ?>" />
  </div>
 </div>
 <div class="form-group">
  <label for="isi" class='control-label col-sm-2'>Isi Halaman</label>
  <div class="col-sm-4">
   <textarea name="isi" id="isi" class="tinymce"><?= html_entity_decode($row['isi_artikel']) ?></textarea>
  </div>
 </div>
